...
public class Paginate {
     ...
     * @param command       The command, not including page number, to use for moving between pages.
     * @return              The created page ready for TextUtils
     */
    public String createPage(List<String> lines, int pageNumber, String command) {
        String page;
        String prevPage = currentPage > 1 ? "{$(click:run_command,/" + command + " " + (pageNumber - 1) +
            ")$(hover:show_text&(gold)Click to view the previous page)&(gold)[{&(aqua)Prev}]) }"
            : "{&(gold)[{&(gray)Prev}] }";
        String title = "&(gold)" + this.header + " - Page " + pageNumber + "/" + pages;
        String nextPage = this.currentPage == pages ? " &(gold)[{&(gray)Next}]"
            : "{ $(click:run_command,/" + command + " " + (pageNumber + 1) +
                ")$(hover:show_text,&(gold)Click to view the next page)&(gold)[{&(aqua)Next}])}";
