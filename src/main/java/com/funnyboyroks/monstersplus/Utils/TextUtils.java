...
public class TextUtils {
    ...
    private static class Parser {
        String cfmt;
        TextUtils.Stack<BuildableTextComponent> stack;

        // Current character index
        int index;

        // Current token being built
        TextUtils.Stack<StringBuffer> tokenStack;

        // Whether or not the component at the current depth according to the variable above
        // has any children appended to it
        TextUtils.Stack<Boolean> hasChildren;

        // Keep track of indices where errors could occur
        TextUtils.Stack<Integer> indexStack;

        Parser(String cfmt) {
            this.cfmt = cfmt;
            stack = new TextUtils.Stack<>(COMPONENT_STACK_ERROR);
            index = 0;
            tokenStack = new TextUtils.Stack<>(TOKEN_STACK_ERROR);
            hasChildren = new TextUtils.Stack<>(CHILD_STACK_ERROR);
            indexStack = new TextUtils.Stack<>(INDEX_STACK_ERROR);
                    ...
                    if ("show_item".equals(eventName)) {
                        try {
                            component.inner.setHoverEvent(new HoverEvent(
                                ...
                                new Gson().fromJson(eventArg, TextUtils.Item.class).into()
                            ));
                        } catch (Throwable unused) {
                        ...
                        try {
                            component.inner.setHoverEvent(new HoverEvent(
                                ...
                                new Gson().fromJson(eventArg, TextUtils.Entity.class).into()
                            ));
                        } catch (Throwable unused) {
    ...
    private static class Entity {
        ...
        net.md_5.bungee.api.chat.hover.content.Entity into() throws TextUtils.ParserError {
            return new net.md_5.bungee.api.chat.hover.content.Entity(
                type,
                id,
                name == null ? null : TextUtils.parse(name, null)[0]
            );
        }
    }
}
